 ...
 */
package org.enterprisedomain.classsupplier.jobs.load;
...
import org.enterprisedomain.classsupplier.impl.ContributionImpl;
import org.enterprisedomain.classsupplier.jobs.ContainerJob;
	...
	private synchronized void doLoad(State state, Bundle osgiBundle) throws Exception {
		try {
			...
			String packageClassName = CodeGenUtil.safeName(model.getName()) + "." //$NON-NLS-1$
					+ CodeGenUtil.capName(CodeGenUtil.safeName(model.getNsPrefix())) + "Package"; //$NON-NLS-1$
